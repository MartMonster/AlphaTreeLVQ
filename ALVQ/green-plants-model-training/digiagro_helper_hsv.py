...
def img_sample(img, n_samples):
    ...
    if n_samples < img_flat.shape[0]:
        idx = np.random.permutation(img_flat.shape[0])[:n_samples]
        img_flat = img_flat[idx]
    else:
        n_samples = img_flat.shape[0]

    return (img_flat, n_samples)
...
def create_samples(images, batch_sz):
    ...
    for i in range(n_classes):
        ...
        for k, (img, weight) in enumerate(images[i]):
            ...
            (img_pix, count) = img_sample(img, count)
...
def data_loader(images, batch_sz):
    while True:    
        ...
        samples[..., 0] = samples[..., 0].astype(np.float32) / 180.0 # H values are in [0, 180] in OpenCV
        samples[..., 1] = samples[..., 1].astype(np.float32) / 255.0 # S values are in [0, 255] in OpenCV
