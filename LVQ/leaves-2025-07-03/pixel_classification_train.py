...
def rgb_uint8_to_feats(samples):
    if (samples.ndim == 2):
        samples = samples[:, None, :]
        
    feats = np.zeros((samples.shape[0], samples.shape[1], 6), dtype=np.float32)
    
    rgb = samples.astype(np.float32) * (1/255)

    M = np.maximum(np.maximum(rgb[..., 0], rgb[..., 1]), rgb[..., 2])
    m = np.minimum(np.minimum(rgb[..., 0], rgb[..., 1]), rgb[..., 2])
    C = M - m
    del m
...
def data_loader(images, batch_sz):
    while True:    
        ...
        samples = rgb_uint8_to_feats(samples)        
