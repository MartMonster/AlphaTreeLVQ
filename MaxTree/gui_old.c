...
GUIInfo *GUIInfoCreate(ubyte *diatompgm, ubyte *shapepgm, long width, long height, MaxTree tree, int filter, int k, int mingreylevel, int numattrs, ProcSet *procs, int *dims, double *dls, double *dhs, 
		       bool sharedcm);
...
void GUIComputeGranulometry(GUIInfo *guiinfo)
{
   ...
   else if (filter==3)  MaxTreeGranulometryMDWilkinson(tree, numattrs, procs, dims, result, dls, dhs);
...
GUIInfo *GUIInfoCreate(ubyte *diatompgm, ubyte *shapepgm, long width, long height, MaxTree tree, int filter,  int k, int mingreylevel,  int numattrs, ProcSet *procs, int *dims, double *dls, double *dhs, bool sharedcm)
{
   GUIInfo *info;
...
void HandleGranEvents(GUIInfo *guiinfo, TLEvent *event)
{
   int x, y;
   char str[100];
   fprintf(stderr, "in HandleGranEvents\n");
   switch(event->Type)
   {
      case TLET_WindowRefresh:
         TLPutImage(guiinfo->GranWindow, guiinfo->GranImage);
      ...
      case TLET_Mouse:
         switch(event->MouseEvent.Action)
         {
            case TLMEA_MousePress:
               fprintf(stderr, "DEBUG: MousePress event at raw=(%d,%d)\n",
               event->MouseEvent.X, event->MouseEvent.Y);
               x = (event->MouseEvent.X)/8;
               y = (event->MouseEvent.Y)/8;
               fprintf(stderr, "DEBUG: Normalized coords (%d,%d), Dims=(%d,%d)\n",
               x, y, guiinfo->Dims[0], guiinfo->Dims[1]);
               if ((x>=0) && (y>=0) && (x<guiinfo->Dims[0]) && (y<guiinfo->Dims[1]))
               {
                  buttonpressed = true;
                  fprintf(stderr, "DEBUG: buttonpressed=TRUE, prev=(%d,%d)\n", x, y);
                  prev_x = x; prev_y = y;
                  outsidegran = false;
                  TLRefreshImage(guiinfo->DiatomWindow, guiinfo->DiatomImage, guiinfo->DiatomPGM);
                  TLRefreshImage(guiinfo->GranWindow, guiinfo->GranImage, guiinfo->GranPGM);
                  fprintf(stderr, "DEBUG: calling Show2DSelGranNodes at (%d,%d)\n", x, y);
                  Show2DSelGranNodes(guiinfo, 0, 1, x, y);
                  fprintf(stderr, "DEBUG: calling TLImageSelectPixel at (%d,%d)\n", x, y);
                  TLImageSelectPixel(guiinfo->GranWindow, guiinfo->GranImage, x, y);
                  fprintf(stderr, "DEBUG: calling TLPutImage\n");
                  TLPutImage(guiinfo->GranWindow, guiinfo->GranImage);
                  fprintf(stderr, "DEBUG: finished drawing pipeline\n");
               }
               break;
            ...
            case TLMEA_MouseMove:
               fprintf(stderr, "DEBUG: Mouse moved at (%d,%d), Dims=(%d,%d)\n",
        x, y, guiinfo->Dims[0], guiinfo->Dims[1]);
	          ...
	          if ((prev_x!=x) || (prev_y!=y))
                  {
                     prev_x = x; prev_y = y;
                     sprintf(str, "Granulometry (%d, %d)", x,y);/*guiinfo->Result[y*(guiinfo->Dims[0]+1)+x]*/
