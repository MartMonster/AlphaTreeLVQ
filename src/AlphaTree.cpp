...
AlphaNode<Pixel>::AlphaNode(Pixel pixelVal, float alpha_, ImgIdx parentidx_)
    : area(1), alpha(alpha_), sumPix((float)pixelVal), minPix(pixelVal), maxPix(pixelVal), parentIdx(parentidx_),
      _rootIdx(ROOTIDX) {
...
template <class Pixel>
void AlphaNode<Pixel>::set(ImgIdx area_in, float level, float sumPix_in, Pixel minPix_in, Pixel maxPix_in) {
    this->area = area_in;
    this->alpha = level;
...
template <class Pixel>
void AlphaTree<Pixel>::computeDimgRadixf(RankItem<float> *&rankitem, const Pixel *img, SortValue<float> *&vals) {
    const int shamt = _connectivity >> 2;
    if (_connectivity == 4) {
#pragma omp parallel for schedule(guided, 1)
        for (ImgIdx i = 0; i < _height; i++) {
            ImgIdx imgidx = i * _width;
            ImgIdx dimgidx = imgidx << shamt;
            ...
            if (i < _height - 1) {
                for (ImgIdx j = 0; j < _width - 1; j++) {
                    ...
                    rankitem[contidx] = RankItem<float>(dimgidx++, vals[contidx].val_);
                    contidx++;
        ...
        for (ImgIdx i = 0; i < _height; i++) {
            ...
            ImgIdx contidx = dimgidx - 3 * i - (top ? _width - 1 : 0);
            for (ImgIdx j = 0; j < _width; j++, imgidx++) {
                ...
                if (bottom && right) {
                    vals[contidx].val_ = (float)_pixelDissim.computeDissimilarity(imgidx, imgidx + _width + 1);
                    rankitem[contidx] = RankItem<float>(dimgidx, vals[contidx].val_);
                    contidx++;
                }
                dimgidx++;
...
template <class Pixel> Pixel AlphaTree<Pixel>::compute_dimg(float *dimg, const Pixel *img) {
    using Value = float;
    ...
    if (_connectivity == 4) {
        if (_channel == 1) {
            ...
            for (int ch = 0; ch < _channel; ch++) {
                ...
                for (i = 0; i < _height - 1; i++) {
                    for (j = 0; j < _width - 1; j++) {
                        d = (float)pimg[imgidx + stride_w] - (float)pimg[imgidx];
                        if (ch == 0)
                            dimg[dimgidx] = d * d;
                        ...
                        d = (float)pimg[imgidx + 1] - (float)pimg[imgidx];
                        if (ch == 0)
                            dimg[dimgidx] = d * d;
                        else if (ch != _channel - 1)
                            dimg[dimgidx] += d * d;
...
template <class Pixel>
void AlphaTree<Pixel>::compute_dimg(ImgIdx &minidx, float &mindiff, Pixel *dimg, ImgIdx *dhist, const Pixel *img,
                                    double a) {
    ImgIdx dimgidx, imgidx, stride_w = _width, i, j;
    int hidx;
    mindiff = (float)((Pixel)(-1));
    imgidx = dimgidx = 0;
    ...
    if (_connectivity == 4) {
        for (i = 0; i < _height - 1; i++) {
            for (j = 0; j < _width - 1; j++) {
                ...
                hidx = (int)(a * log2f(1 + (float)dimg[dimgidx++]));
                dhist[hidx]++;
...
template <class Pixel> float AlphaTree<Pixel>::compute_dimg(float *dimg, ImgIdx *dhist, const Pixel *img) {
    ImgIdx dimgidx, imgidx, stride_w = _width, i, j;
    Pixel d;
    float dmax = 0;
...
template <class Pixel> void AlphaTree<Pixel>::FloodHeapQueueNoCache(const Pixel *img) {
    ...
    dimg = (float *)Malloc((size_t)dimgSize * sizeof(float));
    ...
    queue = new HeapQueue<float>(nredges);
    _curSize = 0;
    ...
    stackTop = NewAlphaNode(); /*dummy root*/
    AlphaNode<Pixel> *pNode = _node + stackTop;
    pNode->set(0, currentLevel, (float)0.0, (Pixel)max_level, (Pixel)0);
    pNode->parentIdx = stackTop;
    // currentLevel = max_level;
...
template <class Pixel> void AlphaTree<Pixel>::FloodHeapQueueNaiveNoCache(const Pixel *img) {
    ...
    // create heap-based priority queue
    queue = new HeapQueue_naive<float>(nredges + 1);
    _curSize = 0;
...
template <class Pixel> void AlphaTree<Pixel>::FloodHeapQueue(const Pixel *img) {
    ...
    // create heap-based priority queue
    queue = new Cache_Quad_Heapqueue<float>(nredges);
    _curSize = 0;
    ...
    x0 = 0; /*arbitrary starting point*/
    currentLevel = std::numeric_limits<float>::infinity();
    stackTop = NewAlphaNode(); /*dummy root*/
    AlphaNode<Pixel> *pNode = _node + stackTop;
    pNode->set(0, currentLevel, (float)0.0, (Pixel)max_level, (Pixel)0);
    pNode->parentIdx = stackTop;
...
template <class Pixel> void AlphaTree<Pixel>::FloodHierarQueue(const Pixel *img) {
    ...
    const uint64_t max_level = compute_dimg1(dimg, dhist, img); // calculate pixel differences and make histogram
    const uint64_t numlevels = max_level + 1;

    // create hierarchical queue from dhist
    HierarQueueCache<Pixel> *queue =
        new HierarQueueCache<Pixel>(nredges + 1, dhist, numlevels); // +1 for the dummy _node
    _curSize = 0;
    ...
    AlphaNode<Pixel> *pNode = _node + stackTop;
    pNode->set(0, (Pixel)max_level, (float)0.0, (Pixel)max_level, (Pixel)0);
    pNode->parentIdx = stackTop;
    uint64_t currentLevel = max_level;
    const ImgIdx x0 = 0; /*arbitrary starting point*/
    ImgIdx prevTop = stackTop;

    queue->push_1stitem(x0, currentLevel);
    while (true) {                                                   // Main flooding loop
        while ((int32_t)queue->top_alpha() <= (int32_t)currentLevel) // flood all levels below currentLevel
        {
            ...
            if (currentLevel > (uint64_t)queue->top_alpha()) // go to lower level
            {
                ...
                const ImgIdx newLowerLevel = NewAlphaNode();
                _node[newLowerLevel].set(1, currentLevel, (float)pix_val, pix_val, pix_val);
                _node[newLowerLevel].parentIdx = stackTop;
                _node[newLowerLevel]._rootIdx = ROOTIDX;
                stackTop = newLowerLevel;
                if (isZeroCC) {
                    const ImgIdx newSingletonNodeIdx = NewAlphaNode(0, _node + stackTop);
                    _node[newSingletonNodeIdx].parentIdx = stackTop;
                    _node[newSingletonNodeIdx]._rootIdx = ROOTIDX;
                    _parentAry[p] = newSingletonNodeIdx;
                    prevTop = newSingletonNodeIdx;
                } else
                    _parentAry[p] = stackTop;
            } else {
                if (isZeroCC) {
                    Pixel pix_val = img[p];
                    const ImgIdx newSingletonNodeIdx = NewAlphaNode();
                    _node[newSingletonNodeIdx].set(1, 0, (float)pix_val, pix_val, pix_val);
                    _node[stackTop].add(_node + newSingletonNodeIdx);
                    _node[newSingletonNodeIdx].parentIdx = stackTop;
                    _node[newSingletonNodeIdx]._rootIdx = ROOTIDX;
                    _parentAry[p] = newSingletonNodeIdx;
                } else
...
void AlphaTree<Pixel>::FloodHierarHeapQueueNoCache(const Pixel *img, double a, double r, int listsize) {
    ...
    max_level = (sizeof(Pixel) == 8) ? 0xffffffffffffffff : (int64_t)((Pixel)(-1));
    numlevels = (uint64_t)(a * log2f(1 + (float)max_level)) + 1;
    ...
    stackTop = NewAlphaNode(); /*dummy root*/
    AlphaNode<Pixel> *pNode = _node + stackTop;
    pNode->set(0, (float)max_level, (float)0.0, (Pixel)max_level, (Pixel)0);
    pNode->parentIdx = stackTop;
    currentLevel = (float)max_level;
    ...
    while (true) // flooding
    {
        while (firstpixel || (float)queue->top_alpha() <= (float)currentLevel) // flood all levels below currentLevel
        {
            if (!firstpixel) {
            ...
            if ((float)currentLevel > (float)queue->top_alpha()) // go to lower level
            {
                Pixel pix_val = img[p];
                currentLevel = queue->top_alpha();
                iNode = NewAlphaNode();
        ...
        if ((float)queue->top_alpha() < (float)_node[parentNodeIdx].alpha) {
...
template <class Pixel>
void AlphaTree<Pixel>::initialize_node1(const Pixel *img, RankItem<float> *rankitem, Pixel maxpixval) {
    ImgIdx p, imgSize = _width * _height;
...
void AlphaTree<Pixel>::initialize_node1(const Pixel *img, RankItem<float> *rankitem, Pixel maxpixval,
    ...
    for (p = 0; p < _maxSize; p++) {
        ImgIdx q = p;
        if (p < imgSize)
            _node[p].set(1, 0, (float)img[p], img[p], img[p]);
        else {
...
template <class Pixel>
void AlphaTree<Pixel>::initialize_node_par1(const Pixel *img, RankItem<float> *rankitem, Pixel maxpixval,
                                            int32_t *rankToIndex) {
    ImgIdx p, imgSize = _width * _height;
...
template <class Pixel> void AlphaTree<Pixel>::FloodTrieHypergraph(const Pixel *img) {
    ...
    maxpixval = ~(1 << nbits);
    rankitem = (RankItem<float> *)Malloc(nredges * sizeof(RankItem<float>));
    _parentAry = 0;
...
// test quantize_ranks_compute_histogram. make sure thing are run in parallel (parallel for loop check etc.)
...
#pragma omp parallel for
    for (int blk = 0; blk < numpartitions; blk++) {
        const ImgIdx blockWidth = blockWidths[blk];
        const ImgIdx blockHeight = blockHeights[blk];
        const bool topBlock = (blk / npartition_x) > 0;
        const bool rightBlock = (blk % npartition_x) < (npartition_x - 1);
        const bool bottomBlock = (blk / npartition_x) < (npartition_y - 1);
        const ImgIdx startPixelIndex = startpidx[blk];
        const int shamt = _connectivity / 4;
        ImgIdx *blockDiffHist = dhist + numbins * blk;
        Pixel maxdiff = 0;
        ...
        if (_connectivity == 4) {
            for (ImgIdx i = 0; i < blockHeight; i++) {
                ...
                for (ImgIdx j = 0; j < blockWidth; j++) {
                    ...
                    const ImgIdx p = startPixelIndex + i * _width + j;
                    const ImgIdx q = p << shamt;
                    if (bottomBlock || bottom) {
                        const uint8_t qr = QUANTIZE_RANK(rank[q], binsize);
                        qrank[q] = qr;
                        blockDiffHist[qr]++;
                        if (bottom) // maxdiff is for max queue level for PQ in subblock flooding, thus inter-block
                                    // edges are ignored
                            maxdiff = _max(maxdiff, qr);
                    }
                    if (rightBlock || right) {
                        const uint8_t qr = QUANTIZE_RANK(rank[q + 1], binsize);
                        qrank[q + 1] = qr;
                        blockDiffHist[qr]++;
                        if (right)
                            maxdiff = _max(maxdiff, qr);
            ...
            for (ImgIdx i = 0; i < blockHeight; i++) {
                ...
                for (ImgIdx j = 0; j < blockWidth; j++) {
                    ...
                    if ((bottomBlock && rightBlock) || (bottom && right)) {
                        const uint8_t qr = QUANTIZE_RANK(rank[q + 1], binsize);
                        qrank[q + 1] = qr;
                        blockDiffHist[qr]++;
                        if (bottom && right)
                    ...
                    if (right || rightBlock) {
                        const uint8_t qr = QUANTIZE_RANK(rank[q + 2], binsize);
                        qrank[q + 2] = qr;
                        blockDiffHist[qr]++;
                        if (right)
                            maxdiff = _max(maxdiff, qr);
                    }
                    if ((topBlock && rightBlock) || (top && right)) {
                        const uint8_t qr = QUANTIZE_RANK(rank[q + 3], binsize);
                        qrank[q + 3] = qr;
                        blockDiffHist[qr]++;
                        if (top && right)
                            maxdiff = _max(maxdiff, qr);
...
void AlphaTree<Pixel>::unionfind_refine_qlevel(int64_t qlevel, int64_t binsize, ImgIdx nredges,
                                               AlphaNode<Pixel> *pilottree, RankItem<float> *rankitem,
                                               int8_t *redundant_edge, int32_t *rankToIndex) {
    const ImgIdx rank_start = (ImgIdx)qlevel * (ImgIdx)binsize;
    const ImgIdx rank_end = std::min(nredges - 1, ((ImgIdx)qlevel + 1) * (ImgIdx)binsize - 1);
    const ImgIdx imgSize = _width * _height;
    ...
    if (qlevel) {
        for (ImgIdx r = rank_start; r <= rank_end; r++) {
            ...
            const RankItem<float> *pRank = rankitem + ridx;

            const ImgIdx nodeaddr = r + imgSize;

            if (redundant_edge && redundant_edge[rankitem[ridx].dimgidx])
                continue;

            const ImgIdx x0 = find_root(pilottree, pRank->get_pidx0(_connectivity), qlevel);
            const ImgIdx y0 = find_root(pilottree, pRank->get_pidx1(_width, _connectivity), qlevel);

            if (x0 == y0) // already connected, nothing to do
                continue;

            const ImgIdx x = find_root(pilottree[x0]._rootIdx);
            const ImgIdx y = find_root(pilottree[y0]._rootIdx);
        ...
        for (ImgIdx r = rank_start; r <= rank_end; r++) {
            const RankItem<float> *pRank = rankitem + (rankToIndex ? rankToIndex[r] : r);

            const ImgIdx nodeaddr = r + imgSize;

            // find subtree roots from two edge incidents
            const ImgIdx x0 = pRank->get_pidx0(_connectivity);
            const ImgIdx y0 = pRank->get_pidx1(_width, _connectivity);

            const ImgIdx x = find_root(_node[x0]._rootIdx);
            const ImgIdx y = find_root(_node[y0]._rootIdx);
...
template <class Pixel> void AlphaTree<Pixel>::connect_pilotnode(AlphaNode<Pixel> *pilottree) {
    const ImgIdx imgSize = _width * _height;
...
void AlphaTree<Pixel>::set_subblock_properties(ImgIdx *startpidx, ImgIdx *blockWidths, ImgIdx *blockHeights,
    ...
    const ImgIdx nrEdgePerPixel = _connectivity / 2;
    ImgIdx p = 0;
    for (int8_t y = 0; y < npartition_y; y++) {
        ...
        for (int8_t x = 0; x < npartition_x; x++) {
            ...
            const bool lastcol = (x == npartition_x - 1);
            const ImgIdx blockWidth = lastcol ? blksz_xn : blksz_x;
            blocksize[p] = blockHeight * blockWidth * nrEdgePerPixel;
            // clang-format off
            if (_connectivity == 4) {
                //   -  -  -
                //   -  p  1
                //   -  0  -
                if (lastrow) blocksize[p] -= blockWidth;
                if (lastcol) blocksize[p] -= blockHeight;
            } else if (_connectivity == 8) {
                //   -  -  3
                //   -  p  2
                //   -  0  1
                if (firstrow) blocksize[p] -= blockWidth;
                if (lastcol)  blocksize[p] -= 3 * blockHeight;
                if (lastrow)  blocksize[p] -= 2 * blockWidth;
                blocksize[p] += (ImgIdx)(firstrow && lastcol) + (ImgIdx)(lastrow && lastcol);
            }
            // clang-format on
            blockWidths[p] = blockWidth;
            blockHeights[p] = blockHeight;
...
template <class Pixel>
void AlphaTree<Pixel>::computeDimgAndRankToIndex(RankItem<float> *&rankitem, const Pixel *img, ImgIdx nredges,
                                                 int32_t *rankToIndex) {
...
template <class Pixel>
void AlphaTree<Pixel>::compute_difference_and_sort(RankItem<float> *&rankitem, const Pixel *img, ImgIdx nredges) {
    int32_t *rankToIndex = (int32_t *)Calloc(nredges * sizeof(int32_t));

    computeDimgAndRankToIndex(rankitem, img, nredges, rankToIndex);

    RankItem<float> *sorteditem = (RankItem<float> *)Malloc(nredges * sizeof(RankItem<float>)), *tmp;
#pragma omp parallel for schedule(guided, 1)
...
template <class Pixel>
void AlphaTree<Pixel>::computeDifferenceAndSort(ImgIdx *rank, RankItem<float> *&rankitem, const Pixel *img,
...
std::pair<ImgIdx, uint8_t> AlphaTree<Pixel>::getFloodingContext(bool &isFirstPixel, const ImgIdx &qitem,
    ...
    uint8_t isAv;
    if (_connectivity == 4) {
        static const ImgIdx edgeStrides[2] = {_width, 1};
        static const ImgIdx isAvMasks[2][2] = {{0b1110, 0b0111}, {0b1101, 0b1011}};
        p += (ImgIdx)edgeIncident * edgeStrides[edgeDirection];
        isAv = isAvailable[p] & isAvMasks[edgeDirection][edgeIncident];
    } else {
        static const ImgIdx edgeStrides[4] = {_width, _width + 1, 1, -_width + 1};
        static const ImgIdx isAvMasks[4][2] = {
            {0b11111110, 0b11101111}, {0b11111101, 0b11011111}, {0b11111011, 0b10111111}, {0b11110111, 0b01111111}};
        p += (ImgIdx)edgeIncident * edgeStrides[edgeDirection];
        isAv = isAvailable[p] & isAvMasks[edgeDirection][edgeIncident];
    }
...
void AlphaTree<Pixel>::markRedundantEdges(int8_t *redundant_edge, const ImgIdx &p, const uint8_t &connected_neighbor,
    ...
    if (_connectivity == 4) {
        ...
        if (connected_neighbor & 0b00000001) {
            const ImgIdx leaf2 = _parentAry[p + _width];
            if (qrank[q] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q] = 1;
        }
        if (connected_neighbor & 0b00000010) {
            const ImgIdx leaf2 = _parentAry[p + _width + 1];
            if (qrank[q + 1] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q + 1] = 1;
        }
        if (connected_neighbor & 0b00000100) {
            const ImgIdx leaf2 = _parentAry[p + 1];
            if (qrank[q + 2] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q + 2] = 1;
        }
        if (connected_neighbor & 0b00001000) {
            const ImgIdx leaf2 = _parentAry[p - _width + 1];
            if (qrank[q + 3] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q + 3] = 1;
        }
        if (connected_neighbor & 0b00010000) {
            const ImgIdx leaf2 = _parentAry[p - _width];
            if (qrank[q - wstride_d] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q - wstride_d] = 1;
        }
        if (connected_neighbor & 0b00100000) {
            const ImgIdx leaf2 = _parentAry[p - _width - 1];
            if (qrank[q - wstride_d - 3] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q - wstride_d - 3] = 1;
        }
        if (connected_neighbor & 0b01000000) {
            const ImgIdx leaf2 = _parentAry[p - 1];
            if (qrank[q - 2] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q - 2] = 1;
        }
        if (connected_neighbor & 0b10000000) {
            const ImgIdx leaf2 = _parentAry[p + _width - 1];
            if (qrank[q + wstride_d - 1] > get_nearest_common_ancestor_level(p, leaf2))
                redundant_edge[q + wstride_d - 1] = 1;
        }
    }
...
template <class Pixel>
void AlphaTree<Pixel>::HybridPilotFlooding(const Pixel *img, ImgIdx *startpidx, int8_t *redundant_edge,
                                           ImgIdx numpartitions, ImgIdx *blocksize, ImgIdx *blockWidths,
                                           ImgIdx *blockHeights, ImgIdx numlevels, ImgIdx *subtree_cur,
                                           ImgIdx *subtree_max, ImgIdx *dhist, uint8_t *qrank, ImgIdx blksz_x,
...
#pragma omp parallel for schedule(dynamic, 1)
    for (int blk = 0; blk < numpartitions; blk++) {
        ...
        queue->set_queue(dhist + numlevels * blk);

        ImgIdx stackTop = nodeIdx++;
        ImgIdx prevTop = stackTop;
        Pixel currentLevel = subtree_max[blk];
        ...
        while (_node[stackTop].area < blockArea) {                       // flooding
            while ((int64_t)queue->min_level <= (int64_t)currentLevel) { // flood all levels below currentLevel
                ...
                const ImgIdx wstride = _width << shamt;
                auto packEdgeIncidence = [](const ImgIdx &edgeIndex, bool edgeIncidence) -> ImgIdx {
                    if (edgeIncidence)
                        return (edgeIndex << 1) | (ImgIdx)0b1;
                    else
                        return (edgeIndex << 1) & (ImgIdx)~0b1;
                };
                ...
                if (redundant_edge && connected_neighbor)
...
// hp
template <class Pixel> void AlphaTree<Pixel>::HybridParallelNoRedundantSkip(const Pixel *img, int numthreads) {
    clear();
    const ImgIdx imgSize = _width * _height;
    const ImgIdx nredges = _width * (_height - 1) + (_width - 1) * _height +
                           ((_connectivity == 8) ? ((_width - 1) * (_height - 1) * 2) : 0);
    const ImgIdx dimgSize = (_connectivity >> 1) * _width * _height;
    RankItem<float> *rankitem = (RankItem<float> *)Malloc(nredges * sizeof(RankItem<float>));
    ...
    // ImgIdx numpartitionsTarget = _min(imgSize / 2, _min(256, numthreads * 3));
    ImgIdx numpartitionsTarget = _min(imgSize / 2, _min(256, numthreads)); // TODO: study numtrhead constandst
    const auto [npartition_x, npartition_y] = computePartitionSize(numpartitionsTarget);

    ImgIdx binsize = nredges / numpartitionsTarget;
    const ImgIdx numbins = (nredges + binsize - 1) / binsize;
    const ImgIdx blksz_x = _width / npartition_x;
    const ImgIdx blksz_y = _height / npartition_y;
    const ImgIdx blksz_xn = blksz_x + (_width % npartition_x);
    const ImgIdx blksz_yn = blksz_y + (_height % npartition_y);
    const ImgIdx numpartitions = (int64_t)npartition_x * (int64_t)npartition_y;
    ...
    int8_t *redundant_edge = nullptr; // (int8_t *)Calloc(dimgSize * sizeof(int8_t));

    HybridPilotFlooding(img, startpidx, redundant_edge, numpartitions, blocksize, blockWidths, blockHeights, numbins,
                        subtree_cur, subtree_max, dhist, qrank, blksz_x, blksz_y, npartition_x, npartition_y);
...
#pragma omp parallel for schedule(dynamic, 1)
    for (ImgIdx qlevel = 0; qlevel < numbins; qlevel++) {
...
template <class Pixel> void AlphaTree<Pixel>::HybridParallelOld(const Pixel *img, int numthreads) {
    ...
    const auto [npartition_x, npartition_y] = computePartitionSize(numpartitions);
        ...
        for (int blk = 0; blk < numpartitions; blk++) // flooding is somehow slower than the one without tse...
        {
            ...
            while (1) // flooding
            {
                while ((int64_t)queue->min_level <= (int64_t)currentLevel) // flood all levels below currentLevel
                {
                    ...
                    ImgIdx didx = qitem >> 1;
                    // the pixel which pushed this item into the queue, where it is easier to find the level of the
                    // edge (level = stackTop)
